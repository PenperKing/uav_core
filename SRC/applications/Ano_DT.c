...
void Send_system_data(void)
{
	...
	System_data.Acc_x = sensor.Acc[X];
	System_data.Acc_y = sensor.Acc[Y];
	System_data.Acc_z = sensor.Acc[Z];
	
	System_data.Gyro_x = (short)(angel_Acc_out[0]*100);
	System_data.Gyro_y = (short)(angel_Acc_out[1]*100);
	System_data.Gyro_z = (short)(angel_Acc_out[2]*100);
	
	System_data.Roll  = (short)(imu_data.rol*100);
	System_data.Pitch = (short)(imu_data.pit*100);
	System_data.Yaw   = (short)(imu_data.yaw*100);
											
									
	System_data.tag_num = uwb_location.tag_num;
	System_data.UWB_x = uwb_location.x;
	System_data.UWB_y = uwb_location.y;
	
	System_data.z  = OF_ALT;;
	System_data.dx = OF_DX;
	System_data.dy = OF_DY;
	
	//int ahh=(int)(Plane_Votage*100);
	System_data.power_Votage = (unsigned short)(100*Plane_Votage);
	
	Ano_system_data(&System_data);
}


void Ano_system_data(System_t *data)
{
	u8 _cnt=0;
	vs16 _temp;
	vs16 _temp1;
	...
	// uwb tag
	data_to_send[_cnt++]=BYTE0(data->tag_num);
